...
没有人能独善其身所以没有directexport这一说,大家都是要做改变的.



我认为暴露哪些API是根据用户使用层次决定的

比如说用户想要简简单单配置一个服务的话,而不去自己扩展可扩展点的话

那么对于这类用户,我们只需要暴露给他相关的配置类,就是config包下的ServciceConfig这些,牵扯到的对于URL和Invoker这些依赖没必要暴露，也就是不暴露那些含有对内部有依赖的方法，我觉得这个完全ok.



但是您说还要暴露带有SPI接口的类，我想您的意思是向让用户拥有扩展扩展点的能力，
...
1.   哪些类，config包下的，不含有内部类的。
2.   对于类，暴露哪些方法，签名上不含有内部东西的。

策略

1.   Enum和Annonatation直接挪出
2.   接口扫描后挪出。

内部外部实际上都得做扫描。

得有一个codeBlockChanger。
...
```
/*
现在改变原有的思路，只暴露方法签名中没有未暴露的暴露的方法。
相应的要改变这些地方。
1. extra实际上还是有的会因为这些原因而产生
    1. 直接暴露的类中有牵连，比如说含有内部类，接口这些类
    2. 继承泛型这些。
2. 对于内部类的处理我们这样做。
    直接挪出去吧，对其牵扯到的东西按原计划处理。
那么现在如何安排代码比较合理呢
1. init不变依旧是做这些事情，

今天是7号，十号之前做出一个东西，得能跑。
可以看作代码生成出原来的demo

 */
 好像暂时只能做到一点就是彻底不暴露返回值有dubbo的。
 先这样搞吧，实在没有啥别的办法了呀。
 那接口的又要怎么办啊
```

先想好要干啥，从最简略设计，我只想暴露很少一部分。

对于config这些，方法中就不要暴露了 ， 接口也一样，主要是不想为接口生成类。含有内部类的直接不暴露虽然很操蛋。但是现在也只能这样了。
