...
public final class ClassGenerator {
    ...
    public ClassGenerator addField(String name, int mod, Class<?> type, String def) {
        ...
        sb.append(modifier(mod)).append(' ').append(ByteCodeUtils.getName(type)).append(' ');
        sb.append(name);
        if (ByteCodeUtils.isNotEmpty(def)) {
            sb.append('=');
            sb.append(def);
        }
        sb.append(';');
    ...
    public ClassGenerator addMethod(String name, int mod, Class<?> rt, Class<?>[] pts, Class<?>[] ets,
                                    String body) {
        StringBuilder sb = new StringBuilder();
        sb.append(modifier(mod)).append(' ').append(ByteCodeUtils.getName(rt)).append(' ').append(name);
        sb.append('(');
        for (int i = 0; i < pts.length; i++) {
            if (i > 0) {
                sb.append(',');
            }
            sb.append(ByteCodeUtils.getName(pts[i]));
            sb.append(" arg").append(i);
        }
        sb.append(')');
        if (ByteCodeUtils.isNotEmpty(ets)) {
            sb.append(" throws ");
            for (int i = 0; i < ets.length; i++) {
                if (i > 0) {
                    sb.append(',');
                }
                sb.append(ByteCodeUtils.getName(ets[i]));
            }
        }
    ...
    public ClassGenerator addMethod(String name, Method m) {
        String desc = name + ByteCodeUtils.getDescWithoutMethodName(m);
        addMethod(':' + desc);
    ...
    public ClassGenerator addConstructor(Constructor<?> c) {
        String desc = ByteCodeUtils.getDesc(c);
        addConstructor(":" + desc);
    ...
    private CtMethod getCtMethod(Method m) throws NotFoundException {
        return getCtClass(m.getDeclaringClass())
                .getMethod(m.getName(), ByteCodeUtils.getDescWithoutMethodName(m));
    }

    private CtConstructor getCtConstructor(Constructor<?> c) throws NotFoundException {
        return getCtClass(c.getDeclaringClass()).getConstructor(ByteCodeUtils.getDesc(c));
    }
