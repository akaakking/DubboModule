...
public abstract class Wrapper {
    ...
    private static Wrapper makeWrapper(Class<?> c) {
        ...
        ClassLoader cl = ByteCodeUtils.getClassLoader(c);
        ...
        try {
            ...
            for (CtMethod method : ctMethods) {
                allMethod.add(ByteCodeUtils.getDesc(method));
            }
        } catch (Exception e) {
        ...
        Method[] methods = Arrays.stream(c.getMethods())
                .filter(method -> allMethod.contains(ByteCodeUtils.getDesc(method)))
                .collect(Collectors.toList())
                .toArray(new Method[] {});
        // get all public method.
        boolean hasMethod = ByteCodeUtils.hasMethods(methods);
        if (hasMethod) {
            ...
            for (Method m : methods) {
                ...
                ms.put(ByteCodeUtils.getDesc(m), m);
            }
        ...
        for (Map.Entry<String, Method> entry : ms.entrySet()) {
            ...
            if ((matcher = ByteCodeUtils.GETTER_METHOD_DESC_PATTERN.matcher(md)).matches()) {
                String pn = propertyName(matcher.group(1));
                c2.append(" if( $2.equals(\"").append(pn).append("\") ){ return ($w)w.").append(method.getName()).append("(); }");
                pts.put(pn, method.getReturnType());
            } else if ((matcher = ByteCodeUtils.IS_HAS_CAN_METHOD_DESC_PATTERN.matcher(md)).matches()) {
                String pn = propertyName(matcher.group(1));
                c2.append(" if( $2.equals(\"").append(pn).append("\") ){ return ($w)w.").append(method.getName()).append("(); }");
                pts.put(pn, method.getReturnType());
            } else if ((matcher = ByteCodeUtils.SETTER_METHOD_DESC_PATTERN.matcher(md)).matches()) {
    ...
    private static String arg(Class<?> cl, String name) {
        ...
        return "(" + ByteCodeUtils.getName(cl) + ")" + name;
    }
